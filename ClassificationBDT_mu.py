...
#    inputS = TFile.Open("root://cms-xrd-global.cern.ch///store/mc/RunIIFall17NanoAODv5/DYJetsToLL_M-5to50_TuneCP5_13TeV-madgraphMLM-pythia8/NANOAODSIM/PU2017_12Apr2018_Nano1June2019_102X_mc2017_realistic_v7-v1/70000/773DAD3C-8145-F643-8934-ACEFB018C1D6.root")
 #   inputB = TFile.Open("root://cms-xrd-global.cern.ch///store/mc/RunIIFall17NanoAODv5/WJetsToLNu_TuneCP5_13TeV-madgraphMLM-pythia8/NANOAODSIM/PU2017_12Apr2018_Nano1June2019_102X_mc2017_realistic_v7-v1/110000/59A3A9A4-660B-7B4F-B858-23CFE7C09303.root")
    inputS = TFile.Open("/afs/cern.ch/work/s/srudrabh/ZH3l/CMSSW_10_2_0/src/LatinoAnalysis/NanoGardener/test/DYJetsToLL_M-5to50_MuPromptGenMatched.root")
    inputBA = TFile.Open("/afs/cern.ch/work/s/srudrabh/ZH3l/CMSSW_10_2_0/src/LatinoAnalysis/NanoGardener/test/WJetsToLNu_MuPromptGenMatched.root")
    inputBB = TFile.Open("/afs/cern.ch/work/s/srudrabh/ZH3l/CMSSW_10_2_0/src/LatinoAnalysis/NanoGardener/test/TTToSemiLeptonic_MuPromptGenMatched.root")
    ...
    dataloader.AddVariable("Muon_jetPtRatio := (Muon_jetIdx == -1)*((Muon_pt)/(Muon_pt+(Muon_miniPFRelIso_all-Muon_miniPFRelIso_chg)))+ (Muon_jetIdx >= 0)*((Muon_pt)/(Jet_pt[max(Muon_jetIdx,0)]))", "Muon_jetPtRatio", 'F') #takes a different value for Electron_jetIdx == -1 and for when Electron_jetIdx >= 0
    ...
    dataloader.SetSignalWeightExpression("(nMuon>0)*(Muon_promptgenmatched)")
    dataloader.SetBackgroundWeightExpression("(nMuon>0)*(!Muon_promptgenmatched)")
    mycuts = TCut("")
    mycutb = TCut("")

    dataloader.PrepareTrainingAndTestTree(mycuts, mycutb,'nTrain_Signal=0:nTrain_Background=0:nTest_Signal=0:nTest_Background=0:SplitMode=Random:NormMode=NumEvents:!V') #running over 5 events only
